...
const PromotionManagementPage = () => {
    ...
    try {
      const response = await fetch(
        `/api/marketplace/promotion/${updatedPromotion.id}`,
        {
          ...
          body: JSON.stringify({ promoPrice: updatedPromotion.promoPrice }),
        },
      );
  ...
  const handleEdit = (promotion: Promotion) => {
    ...
    setEditingPromotion(promotion); // Set editingPromotion to the selected promotion
  };

  const handleCloseEdit = () => {
    setEditingPromotion(null); // Close edit form by setting editingPromotion to null
  };
