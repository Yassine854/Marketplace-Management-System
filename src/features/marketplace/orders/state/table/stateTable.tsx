...
export default function StateTable({
  ...
  const { editState, deleteState } = useStateActions();

  const handleEdit = (id: string, updatedState: State) => {
    editState(id, updatedState).then(() => {
      refetch();
    });
  };
            ...
            {!isLoading && !error && state?.length === 0 && (
              <tr>
                <td className="px-4 py-4 text-center text-gray-500" colSpan={3}>
                  <p>Aucun state ne correspond à vos critères de recherche.</p>
                </td>
              </tr>
            )}
