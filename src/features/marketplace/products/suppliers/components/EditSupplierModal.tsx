...
interface EditSupplierModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (id: string, supplierData: Partial<Manufacturer>) => Promise<void>;
...
const EditSupplierModal = ({
  ...
  useEffect(() => {
    if (initialData) {
      const { categories, ...rest } = initialData;
      setForm({ ...rest, id: initialData.id ?? "" });
      setLocalError(null);
    }
  }, [initialData]);
  ...
  const handleChange = (
    ...
    if (type === "number") {
      ...
      setForm((prev) => ({
        ...prev,
        categories: Array.from(options, (option) => option.value),
      }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.id || form.id === "undefined" || form.id.length !== 24) {
      setLocalError("Invalid or missing supplier ID. Please try again.");
      return;
    }
    setLocalError(null);
