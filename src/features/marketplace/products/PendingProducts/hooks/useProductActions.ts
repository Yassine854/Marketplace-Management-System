...
export function useProductActions() {
  ...
  const acceptProduct = async (id: string) => {
    setIsLoading(true);
    setError(null);
    setSuccessMessage(null);

    try {
      // Just send the accepted flag
      const response = await axios.patch(`/api/marketplace/products/${id}`, {
        accepted: true,
      });

      if (response.status === 200) {
        const successMsg = "Product accepted successfully!";
        setSuccessMessage(successMsg);
        toast.success(successMsg);
        return true;
      }
