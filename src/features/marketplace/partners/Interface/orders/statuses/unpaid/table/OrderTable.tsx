...
export default function OrderDataTable({
  ...
  useEffect(() => {
    ...
    getPartner();
  }, [fetchPartner]); // Add fetchPartner to the dependency array

  const handlePrint = async (order: VendorOrder) => {
    const enrichedOrder = await fetchVendorOrder(order.id);
    console.log("Enriched order:", enrichedOrder);
    console.log("Current partner data:", partner);
    setPrintOrder(enrichedOrder);
    setIsPrintOpen(true);
  };
  ...
  const columns = useMemo<ColumnDef<VendorOrder, any>[]>(
    ...
    [onToggleStatus, handlePrint], // Add handlePrint to the dependency array
  );
      ...
      <div className="w-full space-y-3">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative max-w-sm flex-1">
            ...
            {(table.getColumn("orderCode")?.getFilterValue() as string) && (
              <button
                onClick={() => table.getColumn("orderCode")?.setFilterValue("")}
