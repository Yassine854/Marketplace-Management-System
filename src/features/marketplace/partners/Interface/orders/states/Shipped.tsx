...
const OrderManagementPage = () => {
  ...
  const [ShippedStateId, setShippedStateId] = useState<string | null>(null);
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Shipped state
      ...
      // Find the Shipped state
      const ShippedState = statesArray.find(
        (state: State) => state.name.toLowerCase() === "shipped",
      );

      if (ShippedState) {
        setShippedStateId(ShippedState.id);
        setFilters((prev) => ({
          ...prev,
          stateId: ShippedState.id,
        }));
      }

      // Find the ReadyToShip state
      const ReadyToShipState = statesArray.find(
        (state: State) => state.name.toLowerCase() === "ready_to_ship",
      );
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Shipped state ID in the request
      if (ShippedStateId) {
        url.searchParams.append("stateId", ShippedStateId);
      }

      // Add other filters except stateId (we're already filtering by Shipped state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Shipped, remove it from the list
      if (updatedOrder.state?.id !== ShippedStateId) {
        setOrders((prev) =>
  ...
  }, [currentPage, debouncedSearchTerm, itemsPerPage, filters, ShippedStateId]);
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== ShippedStateId)} // Hide Shipped state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the Shipped state
                  setFilters({
                    ...newFilters,
                    stateId: ShippedStateId || "",
                  });
                }}
                onReset={() => {
                  setFilters({
