...
const OrderManagementPage = () => {
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Archived state
      ...
      // Find the Archived state
      const ArchivedState = statesArray.find(
        (state: State) => state.name === "Archived",
      );

      if (ArchivedState) {
        setArchivedStateId(ArchivedState.id);
        // Set the stateId filter to the Archived state ID
        setFilters((prev) => ({
          ...prev,
          stateId: ArchivedState.id,
        }));
      }
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Archived state ID in the request
      if (ArchivedStateId) {
        url.searchParams.append("stateId", ArchivedStateId);
      }

      // Add other filters except stateId (we're already filtering by Archived state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Archived, remove it from the list
      if (updatedOrder.state?.id !== ArchivedStateId) {
        setOrders((prev) =>
