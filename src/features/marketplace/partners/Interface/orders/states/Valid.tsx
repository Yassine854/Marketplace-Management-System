...
const OrderManagementPage = () => {
  ...
  const [ValidStateId, setValidStateId] = useState<string | null>(null);
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the valid state
      ...
      // Find the valid state
      const ValidState = statesArray.find(
        (state: State) => state.name === "valid",
      );

      if (ValidState) {
        setValidStateId(ValidState.id);
        // Set the stateId filter to the valid state ID
        setFilters((prev) => ({
          ...prev,
          stateId: ValidState.id,
        }));
      }
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the valid state ID in the request
      if (ValidStateId) {
        url.searchParams.append("stateId", ValidStateId);
      }

      // Add other filters except stateId (we're already filtering by valid state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from valid, remove it from the list
      if (updatedOrder.state?.id !== ValidStateId) {
        setOrders((prev) =>
  ...
  }, [currentPage, debouncedSearchTerm, itemsPerPage, filters, ValidStateId]);
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== ValidStateId)} // Hide valid state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the valid state
                  setFilters({
                    ...newFilters,
