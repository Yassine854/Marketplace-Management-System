...
const OrderManagementPage = () => {
  ...
  const [DeliveredStateId, setDeliveredStateId] = useState<string | null>(null);
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Delivered state
      ...
      // Find the Delivered state
      const DeliveredState = statesArray.find(
        (state: State) => state.name === "Delivered",
      );

      if (DeliveredState) {
        setDeliveredStateId(DeliveredState.id);
        // Set the stateId filter to the Delivered state ID
        setFilters((prev) => ({
          ...prev,
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Delivered state ID in the request
      if (DeliveredStateId) {
        url.searchParams.append("stateId", DeliveredStateId);
      }

      // Add other filters except stateId (we're already filtering by Delivered state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Delivered, remove it from the list
      if (updatedOrder.state?.id !== DeliveredStateId) {
        setOrders((prev) =>
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== DeliveredStateId)} // Hide Delivered state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the Delivered state
                  setFilters({
                    ...newFilters,
                    stateId: DeliveredStateId || "",
                  });
                }}
                onReset={() => {
                  setFilters({
