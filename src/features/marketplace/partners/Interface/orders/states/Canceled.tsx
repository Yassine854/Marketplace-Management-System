...
const OrderManagementPage = () => {
  ...
  const [CanceledStateId, setCanceledStateId] = useState<string | null>(null);
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Canceled state
      ...
      const CanceledState = statesArray.find(
        (state: State) => state.name === "Canceled",
      );

      if (CanceledState) {
        setCanceledStateId(CanceledState.id);
        // Set the stateId filter to the Canceled state ID
        setFilters((prev) => ({
          ...prev,
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Canceled state ID in the request
      if (CanceledStateId) {
        url.searchParams.append("stateId", CanceledStateId);
      }

      // Add other filters except stateId (we're already filtering by Canceled state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Canceled, remove it from the list
      if (updatedOrder.state?.id !== CanceledStateId) {
        setOrders((prev) =>
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== CanceledStateId)} // Hide Canceled state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the Canceled state
                  setFilters({
                    ...newFilters,
