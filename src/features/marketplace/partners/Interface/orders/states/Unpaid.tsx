...
const OrderManagementPage = () => {
  ...
  const [UnpaidStateId, setUnpaidStateId] = useState<string | null>(null);
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Unpaid state
      ...
      // Find the Unpaid state
      const UnpaidState = statesArray.find(
        (state: State) => state.name === "Unpaid",
      );

      if (UnpaidState) {
        setUnpaidStateId(UnpaidState.id);
        // Set the stateId filter to the Unpaid state ID
        setFilters((prev) => ({
          ...prev,
          stateId: UnpaidState.id,
        }));
      }
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Unpaid state ID in the request
      if (UnpaidStateId) {
        url.searchParams.append("stateId", UnpaidStateId);
      }

      // Add other filters except stateId (we're already filtering by Unpaid state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Unpaid, remove it from the list
      if (updatedOrder.state?.id !== UnpaidStateId) {
        setOrders((prev) =>
  ...
  }, [currentPage, debouncedSearchTerm, itemsPerPage, filters, UnpaidStateId]);
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== UnpaidStateId)} // Hide Unpaid state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the Unpaid state
                  setFilters({
                    ...newFilters,
                    stateId: UnpaidStateId || "",
                  });
                }}
                onReset={() => {
                  setFilters({
