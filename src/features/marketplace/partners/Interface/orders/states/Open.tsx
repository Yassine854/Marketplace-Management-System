...
const OrderManagementPage = () => {
  ...
  const loadFilterData = async () => {
    try {
      // First, fetch states to find the Open state
      ...
      // Find the Open state
      const OpenState = statesArray.find(
        (state: State) => state.name.toLowerCase() === "Open",
      );

      if (OpenState) {
        setOpenStateId(OpenState.id);
        // Set the stateId filter to the Open state ID
        setFilters((prev) => ({
          ...prev,
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      // Always include the Open state ID in the request
      if (OpenStateId) {
        url.searchParams.append("stateId", OpenStateId);
      }

      // Add other filters except stateId (we're already filtering by Open state)
  ...
  const handleUpdate = async (updatedOrder: OrderWithRelations) => {
    try {
      ...
      // If the order's state has changed from Open, remove it from the list
      if (updatedOrder.state?.id !== OpenStateId) {
        setOrders((prev) =>
  ...
  }, [currentPage, debouncedSearchTerm, itemsPerPage, filters, OpenStateId]);
        ...
        <div className="relative grid h-full w-full items-center justify-center gap-4">
          <div className="box w-full min-w-[800px] xl:p-14">
            ...
            {showAdvancedFilter && (
              <AdvancedFilter
                statuses={statuses}
                states={states.filter((state) => state.id !== OpenStateId)} // Hide Open state from filter options
                agents={agents}
                customers={customers}
                paymentMethods={paymentMethods}
                filters={filters}
                onFilterChange={(newFilters: OrderFilters) => {
                  // Ensure stateId is always the Open state
                  setFilters({
                    ...newFilters,
                    stateId: OpenStateId || "",
                  });
