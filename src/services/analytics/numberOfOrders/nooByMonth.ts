...
//console.log(fromDate + "rrrr");
    // Check if data exists and has the expected structure
    if (res.data && res.data.days) {

     let l ;
      const entryIndex1 = res.data.days.findIndex((item: { key: any; }) => item.key === fromDate1);
      if (entryIndex1 == -1)  {
        l =0;
...
let updateDayInList = (externalData: any, list: any[]) => {
  try {
    ...
    const currentFormattedDate = `${currentYear}-${currentMonth < 10 ? '0' + currentMonth : currentMonth}-${currentDay < 10 ? '0' + currentDay : currentDay}`;
    const currentFormattedDate1 = `${currentDay < 10 ? '0' + currentDay : currentDay}-${currentMonth < 10 ? '0' + currentMonth : currentMonth}-${currentYear}`;
    ...
    for (let i = 1; i <= 216; i++) {
      ...
      const pastFormattedDate = `${pastYear}-${pastMonth}-${pastDay < 10 ? '0' + pastDay : pastDay}`;
      //console.log(pastDate +"eeeeeeeeee");// Subtract i days

      // Find the list entry for the specific past date
      const entryIndex = list.findIndex(item => item.date === pastFormattedDate);
      let lastDayValue = externalData.days[externalData.days.length - i-1].value;
      if (entryIndex !== -1) {
        // If the entry is found, update the 'day' value with the lastDayValue
        list[entryIndex].numberOfOrders = lastDayValue;
        //console.log(`Updated entry for ${pastFormattedDate}:`, list[entryIndex]);
      } 
    }

    let lastDayValue1 = externalData.days[externalData.days.length-1].value;
    const entryIndex1 = externalData.days.findIndex((item: { key: any; }) => item.key === currentFormattedDate);
    if (entryIndex1 == -1)  lastDayValue1 =externalData.days[externalData.days.length - 1].value;
    // Push the current day data into the list
    list.push({
